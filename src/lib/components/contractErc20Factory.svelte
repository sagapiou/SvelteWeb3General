<script>
  ...
  async function ercCreate() {
    ...
    let tx;
    try {
      tx = await contractToLoad.deployNewERC20Token(
        ercNewName,
        ercNewSymbol,
        ercNewSupply
      );
...
{#if chainSupported}
  <table class=" flex-row w-full">
      ...
      <div class="modal">
        <div class="modal-box w-11/12 max-w-5xl">
          ...
          <div class="modal-action">
            <label for="my-modal-7" class="btn" on:click={toggleMessage}
              >OK</label
            >
          </div>
        </div>
    ...
    <tr>
      ...
      <td class="  ">
        <div
          class="tooltip align-middle"
          data-tip="CREATE THE NEW ERC20 CONTRACT"
        >
          {#if sendTransactionNewContract}
            <button class="btn loading btn-wide align-middle" disabled
              >Create</button
            >
          {:else}
